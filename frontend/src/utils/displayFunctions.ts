...
import type { Creature, SenseEntity, SpeedEntity, SpellCasting, Statblock, Stat } from "./types"
...
export function displayInnateCasting(data: Statblock) : string {
    ...
    if (!data.description.isProperNoun && !sData.displayAsAction) output += `The ${name.toLowerCase()}'s spellcasting ability is ${fullSpellAbilityName(sData.spellCastingAbility)} (spell save DC ${innateDC(data)}, ${innateSpellBonus(data)} to hit with spell attacks). It can innately cast the following spells${componentsString(data)}:`
    else if (!data.description.isProperNoun && sData.displayAsAction) output += `The ${name.toLowerCase()} casts one of the following spells${componentsString(data)} and using ${fullSpellAbilityName(sData.spellCastingAbility)} as the spellcasting ability (spell save DC ${innateDC(data)}, ${innateSpellBonus(data)} to hit with spell attacks).`
    
    else if (data.description.isProperNoun && !sData.displayAsAction) output += `${name}'s spellcasting ability is ${fullSpellAbilityName(sData.spellCastingAbility)} (spell save DC ${innateDC(data)}, ${innateSpellBonus(data)} to hit with spell attacks). ${name} can innately cast the following spells${componentsString(data)}:`
    else if (data.description.isProperNoun && sData.displayAsAction) output += `${name} casts one of the following spells${componentsString(data)} and using ${fullSpellAbilityName(sData.spellCastingAbility)} as the spellcasting ability (spell save DC ${innateDC(data)}, ${innateSpellBonus(data)} to hit with spell attacks).`
    ...
    for (let times in sData.spellList) {
        ...
        output += sData.spellList[times].map(x => x.comment.length > 0 ? `*${x.spell.toLowerCase()} (${x.comment})*` : `*${x.spell.toLowerCase()}*`).sort().join(", ")
    }
    return output;
}
