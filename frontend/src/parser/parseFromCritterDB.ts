...
import { type CasterSpells, type Statblock, type InnateSpellsList, defaultStatblock, spellListFlattened, type SpellSlotEntity, type SkillsEntity, type Stat, type SpellCasting, type InnateSpellsEntity } from "../components/types";
...
export function parseFromCritterDB(data=tData[0] as any) : [Statblock, {[key: string]: string[]} | null] {
    ...
    outputData.spellcasting.casterSpells = (() => {
        ...
        if (!sData) for (let a of data.stats.additionalAbilities) {
            if (a.name.toLowerCase().includes('spellcasting') && !a.name.toLowerCase().includes('innate')) {
                sData = a.description
            }
        }
...
function parseDamageTypes(data: string[]): string[] {
    ...
    for (let d of data) {
        ...
        if (type.includes(" ") && type.includes("bludgeoning") && type.includes("slashing") && type.includes("piercing")) {
            ...
            if (type.includes(" magical")) modifiers += "Magical "
            if (type.includes(" silvered")) modifiers += "Silvered "
            if (type.includes(" adamantine")) modifiers += "Adamantine"
    ...
    return spells.split(', ')
        .map(s => 
            spellListFlattened
                .find(
                    (item: string) => 
                    item.toLowerCase() === s.trim().replace(/[.$*_]/g, '').replace(/\(.+\)/, '').toLowerCase()
                ) 
                || s.trim().replaceAll(/[.$*_]/g, '').replace(/\(.+\)/, '')
                );
}
...
function innateSpellListConstructor(spellString: string) : InnateSpellsEntity[] {
    ...
    for (let sp of spellList) {
        ...
        let spell = spellListFlattened.find((item: string) => 
            item.toLowerCase() === sp.trim().replace(/[.$*_]/g, '').replace(/\(.+\)/, '')
            )
            || sp.trim().replace(/[.$*_]/g, '').replace(/\(.+\)/, '')
        output.push({
            spell: spell,
            comment:  comment
        })
