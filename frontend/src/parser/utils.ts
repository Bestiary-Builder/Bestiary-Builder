...
export function abilityParser(fData: any, activationType: number) : [FeatureEntity[], string[]] {
	...
	for (let f of fData ?? []) {
		...
		output.push({
			name: name,
			description: description.replace(/<avrae hidden>.*?<\/avrae>/gsi, ''),
...
const ATTACK_PARSER_RE = new RegExp(/(?:\*)?(?:\w+ ){1,4}Attack:(?:\*)? (?<attackBonus>[+-]?\d+) to hit, .*?(?:\*)?Hit:(?:\*)? [+-]?(?:\d+ \((?<damageDiceBase>.+?)\)|(?<damageIntBase>\d+)) (?<damageTypeBase>[aA-zZ ]+) damage[., ]??(?: in melee[.,]?? or [+-]?(?:\d+ \((?<damageRangedDice>.+?)\)|(?<damageRangedInt>\d+)) (?<damageTypeRanged>[aA-zZ ]+) damage at range[,.]?)?(?:,? or [+-]?(?:\d+ \((?<damageDiceVers>.+?)\)|(?<damageIntVers>\d+)) (?<damageTypeVers>[aA-zZ ]+) damage if used with two hands(?: to make a melee attack)?)?(?:,? (?:plus|and) [+-]?(?:\d+ \((?<damageBonusDice>.+?)\)|(?<damageBonusInt>\d+)) (?<damageTypeBonus>[aA-zZ ]+) damage)?/, 'i')
...
export function parseDescIntoAutomation(text: string, name = "", activationType : number) : [FeatureEntity["automation"], null | string] {
	...
	if (override_matches.length > 0) {
		...
		return [{"name": name, 
			"automation": [{"type": "target", "target": "each", "effects": 
			[{"type": "damage", "damage": just_damage_match[1] + `[${just_damage_match[2]}]`, "overheal": false}]}, 
			{"type": "text", "text": text, "title": "Effect"}], "_v": 2, "activation_type": activationType}, null]
	}
