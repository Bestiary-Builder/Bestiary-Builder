...
const addAttack = async (name: string, automation: null | AttackModel | AttackModel[], shouldNotify = true, description = "") => {
    ...
    const loader = $loading.show();
    const { success, error } = await useFetch<AutomationWithType>(`/api/automation/add`, "POST", { name, automation, description: description, collectionId: collection.value.id });
    if (success) {
        await getAutomations();
        if (shouldNotify)
...
<template>
    ...
    <div class="content less-wide" v-if="collection">
        ...
        <Draggable :key="Math.random()" :list="data" group="bestiaries" :animation="150" :item-key="getDraggableKey"
            class="attack-container" :handle="store.isMobile ? '.handle' : ''" :disabled="!personal" @change="saveOrder"
            tag="ol">
    ...
    <Modal :show="showSettingsModal" @close="showSettingsModal = false" v-if="collection">
        <template #header>
            Collection settings
        </template>
        <template #body>
            <div class="modal-desc">
                <LabelledComponent title="Name">
                    <input type="text" :maxlength="store.limits?.nameLength" :minlength="store.limits?.nameMin"
                        v-model="settingOptions.name" />
                </LabelledComponent>
                <LabelledComponent title="Description">
                    <Editor v-model="settingOptions.description" />
                </LabelledComponent>
                <LabelledComponent title="Tags">
                    <v-select multiple :options="store.automationTags" v-model="settingOptions.tags" />
                </LabelledComponent>
            </div>
        </template>
        <template #footer>
