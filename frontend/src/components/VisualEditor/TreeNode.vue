...
<template>
	...
	>
		...
		>
			...
			<span
				v-if="['attack', 'condition', 'save', 'ieffect2'].includes(selfType)" class="collapse-button"
				...
				<template v-if="(key as EffectKey) === 'buttons'">
					...
					>
						<div v-for="(button, index) in effect" :key="index" class="button-item">
							...
							>
								<TreeNode
									v-for="buttonNode, idx in (button as ButtonInteraction).automation"
									:key="idx" :data="buttonNode"
									:depth="depth + 2"
