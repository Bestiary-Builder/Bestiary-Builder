...
import NodeAttackRoot from "./Nodes/NodeAttackRoot.vue";

const props = defineProps<{ node: string }>();
const component = computed(() => {
	if (props.node === "text")
		return NodeText;
	if (props.node === "variable")
		return NodeVariable;
	if (props.node === "target")
		return NodeTarget;
	if (props.node === "remove_ieffect")
		return NodeRemoveIEffect;
	if (props.node === "save")
		return NodeSave;
	if (props.node === "attack")
		return NodeAttack;
	if (props.node === "damage")
		return NodeDamage;
	if (props.node === "condition")
		return NodeCondition;
	if (props.node === "temphp")
		return NodeTempHP;
	if (props.node === "roll")
		return NodeRoll;
	if (props.node === "counter")
