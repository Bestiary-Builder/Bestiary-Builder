		...
		return [
			{
				...
				key: `header-${groupKey}`,
				label: (sortMode.value === "activation_type" ? ACTION_TYPE_MAP[Number.parseInt(groupKey)] || "Attack" : (groupKey || `No ${sortMode.value}`)),
				groupKey,
...
<template>
	<div>
		...
		<v-list v-if="flattenedItems.length > 0" density="compact">
			<v-virtual-scroll
				ref="virtualScroll" max-height="500" :items="flattenedItems" item-height="48"
				item-key="key"
			>
					...
					<v-list-item v-else :title="item.data.name" class="multiline-subtitle creature-item">
						<template #default>
							<v-list-item-subtitle>
								<span v-html="md.renderInline(item.data.description || '')" />
							</v-list-item-subtitle>
						</template>
						...
						<template #append>
							...
							<ImportToCharacter
								:automation="item.data.automation"
