...
<template>
	<div>
		...
		<v-row class="mt-4" density="comfortable">
			<v-col cols="4">
				<v-number-input
					v-model="data.defenses.hp.numOfHitDie" label="Hit Die Number" :min="0"
					prepend-inner-icon="mdi:pound"
				/>
			</v-col>
			<v-col cols="4">
				<v-number-input
					v-model.number="data.defenses.hp.sizeOfHitDie" hide-details class="no-native-spinners"
				...
				>
					<template #append-inner>
						...
						<div class="stacked-controls">
							<v-btn
								icon="mdi:chevron-up" size="24" variant="text" density="compact" :ripple="false"
								@click.stop="stepUp"
							/>
							<v-divider length="100%" />
							<v-btn
								icon="mdi:chevron-down" size="24" variant="text" density="compact"
								:ripple="false" @click.stop="stepDown"
							/>
						</div>
					</template>
				</v-number-input>
			...
			<v-col cols="6">
				<v-number-input
					v-model="data.defenses.ac.ac" label="Armor Class" :min="0"
					prepend-inner-icon="mdi:shield"
				/>
			</v-col>
			<v-col cols="6">
				<v-text-field
					v-model="data.defenses.ac.acSource" label="Armor source"
