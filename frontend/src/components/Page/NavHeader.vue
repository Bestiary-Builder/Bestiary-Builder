...
<template>
	<header>
		<nav id="navbar" ref="navbar" class="navbar" :class="{ expanded: isExpanded }">
			...
			<div class="navbar-center">
				<RouterLink v-for="route in $router.options.routes.filter((a) => a.meta?.navbar)" :key="route.name"
					:to="route.path" class="nav-link" @click="isExpanded = false">
					<span class="header-item">{{ route.name }}</span>
				</RouterLink>
			</div>
			<div class="navbar-right" :class="{ settings: store.user !== null }">
				<RouterLink v-if="store.user" class="user" to="/user" @click="isExpanded = false">
					<span>{{ store.user.username }}</span>
					<div class="image-wrapper">
						<img alt="avatar"
							:src="store.user.avatar ? `https://cdn.discordapp.com/avatars/${store.user.id}/${store.user.avatar}.png` : 'https://cdn.discordapp.com/embed/avatars/0.png'">
			...
			<v-icon-btn icon="uil:bars" @click="isExpanded = !isExpanded" text="Toggle navigation bar"
