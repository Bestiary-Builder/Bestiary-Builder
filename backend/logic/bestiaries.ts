	...
	try {
		...
		for (let creatureId of bestiary.creatures) {
			...
			if (caster.casterLevel && caster.castingClass && caster.spellList.flat().length > 0) {
				let output = `${isNoun ? "" : "The "}${name} is a ${nthSuffix(caster.casterLevel)}-level spellcaster. ${isNoun ? "Their" : "Its"} spellcasting ability is ${fullSpellAbilityName(caster.spellCastingAbilityOverride ?? caster.spellCastingAbility)} (spell save DC ${spellDc(false, creature.stats)}, ${spellAttackBonus(false, creature.stats) >= 0 ? "+" : "" }${spellAttackBonus(false, creature.stats)} to hit with spell attacks). ${isNoun ? name : "It"} ${["Sorcerer", "Bard", "Ranger", "Warlock"].includes(caster.castingClass) ? `knows the following ${caster.castingClass.toLowerCase()} spells` : `has the following ${caster.castingClass.toLowerCase()} prepared`}:${!['Ranger', 'Paladin'].includes(caster.castingClass) && caster.spellList[0].length > 0 ? `\n\nCantrips (at will): ${caster.spellList[0].sort().join(", ").toLowerCase()}` : ""}${caster.spellList[1].length > 0 ? `\n\n1st level (${caster.spellSlotList[1]} slots): ${caster.spellList[1].sort().join(", ").toLowerCase()}` : ""}${caster.spellList[2].length > 0 ? `\n\n2nd level (${caster.spellSlotList[2]} slots): ${caster.spellList[2].sort().join(", ").toLowerCase()}` : ""}${caster.spellList[3].length > 0 ? `\n\n3rd level (${caster.spellSlotList[3]} slots): ${caster.spellList[3].sort().join(", ").toLowerCase()}` : ""}${caster.spellList[4].length > 0 ? `\n\n4th level (${caster.spellSlotList[4]} slots): ${caster.spellList[4].sort().join(", ").toLowerCase()}` : ""}${caster.spellList[5].length > 0 ? `\n\n5th level (${caster.spellSlotList[5]} slots): ${caster.spellList[5].sort().join(", ").toLowerCase()}` : ""}${caster.spellList[6].length > 0 ? `\n\n6th level (${caster.spellSlotList[6]} slots): ${caster.spellList[6].sort().join(", ").toLowerCase()}` : ""}${caster.spellList[7].length > 0 ? `\n\n7th level (${caster.spellSlotList[7]} slots): ${caster.spellList[7].sort().join(", ").toLowerCase()}` : ""}${caster.spellList[8].length > 0 ? `\n\n8th level (${caster.spellSlotList[8]} slots): ${caster.spellList[8].sort().join(", ").toLowerCase()}` : ""}${caster.spellList[9].length > 0 ? `\n\n9th level (${caster.spellSlotList[9]} slots): ${caster.spellList[9].sort().join(", ").toLowerCase()}` : ""}
				`.replaceAll("\t", "")
