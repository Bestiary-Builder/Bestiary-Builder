...
function checkLimits(data: Bestiary | Automation | Creature["stats"]["description"]) {
	if (data.name.length > limits.nameLength) return `Name exceeds the character limit of ${limits.nameLength} characters.`;
	if (data.name.length < limits.nameMin) return `Name is less than the minimum character limit of ${limits.nameMin} characters.`;
	if (data.description.length > limits.descriptionLength) return `Description exceeds the character limit of ${limits.descriptionLength} characters.`;
}
